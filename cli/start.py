...
def check_bucket_access(timeout_seconds: int = 10) -> tuple[bool, str]:
    ...
    try:
        ...
        if "NoSuchBucket" in error_msg:
            ...
            return False, f"Cannot connect to S3 endpoint - check S3_ENDPOINTS in .env"
        elif "502" in error_msg or "Backend" in error_msg:
            return False, f"Cannot reach S3 backend - check S3 endpoints in .env"
...
def cmd_start(args: Namespace) -> int:
    ...
    # Start local proxy on coordinator for bucket access check
    print("=== Starting local proxy ===")
    proxy_ok, proxy_msg = ensure_running_local()
    if proxy_ok:
        print(f"  {proxy_msg}")
    else:
        print(f"\nError: {proxy_msg}")
        print("Cannot start local proxy. Check s3pool binary is available.")
        return 1
    print()
    ...
    if bucket_ok:
        ...
        print("  - Local s3pool proxy is running (s3pool proxy --insecure --listen 127.0.0.1:18080)")
